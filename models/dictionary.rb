module Dictionary
  class << self
    ...
    # def four_char_min_words
    #   words = File.open('data/words_with_4_or_more_chars.txt', "r")
    #   parse(words)
    # end

    def words_with_char_count(num)
      words = File.open("data/words_alpha.txt", "r")
      words.readlines.select { |w| w.length == num + 2 }.map { |w| w.gsub("\r\n", "") }.map(&:downcase)
    end

    def words_with_min_char_count(num)
      words = File.open("data/words_alpha.txt", "r")
      words.readlines.select { |w| w.length >= num + 2 }.map { |w| w.gsub("\r\n", "") }.map(&:downcase)
    end

    def words_in_char_count_range(min:, max:)
      words = File.open("data/words_alpha.txt", "r")
    ...
    def parse(words)
      words.readlines.map { |w| w.delete("\n") }
    end
  end
end
